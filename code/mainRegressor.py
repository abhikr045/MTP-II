...
def gazeXY2label(xy, tablet):
	...
	x = np.array(xy[:,0])
	y = np.array(xy[:,1])
	labels = torch.zeros(len(xy), dtype=int)
	labels[:] = 3   # out of screen = 3 (this is the defualt value, if not left or right)
	xBounds = np.linspace(start=minX, stop=maxX, num=3)
	...
	yOnScreen = (y >= minY) & (y <= maxY)
	left = (x >= xBounds[0]) & (x <= xBounds[1]) & yOnScreen
	right = (x >= xBounds[1]) & (x <= xBounds[2]) & yOnScreen
...
def main():
	...
	criterion = nn.CrossEntropyLoss().to(device=GPU_device)
...
def validate(val_loader, model, criterion, epoch):
	...
	for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze) in enumerate(val_loader):
		...
		outputLR = gazeXY2label(output.cpu().numpy(), tablet='HuaweiMediaPadM3Lite10')
		# accuracy = classifAccuracy(torch.argmax(output, dim=1), gaze)
		accuracy = classifAccuracy(outputLR, gaze)
